---
artigo: "1591"
nomen: "Parentesco — Definição"
parte: "Parte Geral"
livro: "IV — " if c['livro'] else ""
titulo: "I — " if c['titulo'] else ""
capitulo: "Disposições Gerais"
secao: ""
disciplina: civil
grupo: "Direito de Família"
natureza: "regra"
tags:
  - codigo-civil
  - civil
  - art1591
  - parte-geral
  - direito-de-familia
data: 2026-05-31
---

# Art. 1591 CC — Parentesco — Definição

> [!info] Localização
> Parte Geral › Capítulo: Disposições Gerais

> [!quote] Texto Legal
> **Art. 1591.** São parentes em linha reta as pessoas que estão umas para com as outras na relação de
ascendentes e descendentes.

## Por que importa?

| Campo | Valor |
|---|---|
| Dispositivo | Art. 1591 do Código Civil (Lei 10.406/2002) |
| Parte | Parte Geral |
| Livro | — |
| Título | — |
| Capítulo | Disposições Gerais |
| Seção | — |
| Grupo temático | Direito de Família |
| Natureza | regra |

> [!tip] Para o Estudo
> Casamento, união estável e família monoparental têm proteção constitucional. Poder familiar × guarda × alimentos: aspectos processuais e materiais.

> [!warning] Atenção — Pegadinha
> Regime de bens legal (supletivo): comunhão parcial de bens (art. 1640).

## Links Relacionados

← [[Art. 1590]] | [[INDEX — Código Civil]] | [[Art. 1592]] →
