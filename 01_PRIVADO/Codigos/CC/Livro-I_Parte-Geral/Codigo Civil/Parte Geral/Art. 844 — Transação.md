---
artigo: "844"
nomen: "Transação"
parte: "Parte Geral"
livro: "I — " if c['livro'] else ""
titulo: "VI — " if c['titulo'] else ""
capitulo: "Da Transação"
secao: ""
disciplina: civil
grupo: "Contratos em Espécie"
natureza: "regra"
tags:
  - codigo-civil
  - civil
  - art844
  - parte-geral
  - contratos-em-especie
data: 2026-05-31
---

# Art. 844 CC — Transação

> [!info] Localização
> Parte Geral › Capítulo: Da Transação

> [!quote] Texto Legal
> **Art. 844.** A transação não aproveita, nem prejudica senão aos que nela intervierem, ainda que diga
respeito a coisa indivisível.
1
o
Se for concluída entre o credor e o devedor, desobrigará o fiador.
2
o
Se entre um dos credores solidários e o devedor, extingue a obrigação deste para com os

outros credores.
3
o
Se entre um dos devedores solidários e seu credor, extingue a dívida em relação aos co-
devedores.

## Por que importa?

| Campo | Valor |
|---|---|
| Dispositivo | Art. 844 do Código Civil (Lei 10.406/2002) |
| Parte | Parte Geral |
| Livro | — |
| Título | — |
| Capítulo | Da Transação |
| Seção | — |
| Grupo temático | Contratos em Espécie |
| Natureza | regra |

> [!tip] Para o Estudo
> Identifique o contrato típico: elementos essenciais, obrigações das partes, extinção e responsabilidades.

> [!warning] Atenção — Pegadinha
> Cada contrato tem regramento próprio — verifique aplicabilidade subsidiária da PG.

## Links Relacionados

← [[Art. 843]] | [[INDEX — Código Civil]] | [[Art. 845]] →
