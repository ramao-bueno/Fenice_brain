---
artigo: "310"
nomen: "Daqueles a Quem se Deve Pagar"
parte: "Parte Geral"
livro: "I — " if c['livro'] else ""
titulo: "III — Do Adimplemento e Extinção das Obrigações" if c['titulo'] else ""
capitulo: "Do Pagamento"
secao: "Daqueles a Quem se Deve Pagar"
disciplina: civil
grupo: "Obrigações — Modalidades e Transmissão"
natureza: "regra"
tags:
  - codigo-civil
  - civil
  - art310
  - parte-geral
  - obrigacoes-modalidades-e-transmissao
data: 2026-05-31
---

# Art. 310 CC — Daqueles a Quem se Deve Pagar

> [!info] Localização
> Parte Geral › Título III — Do Adimplemento e Extinção das Obrigações › Capítulo: Do Pagamento › Seção: Daqueles a Quem se Deve Pagar

> [!quote] Texto Legal
> **Art. 310.** Não vale o pagamento cientemente feito ao credor incapaz de quitar, se o devedor não provar
que em benefício dele efetivamente reverteu.

## Por que importa?

| Campo | Valor |
|---|---|
| Dispositivo | Art. 310 do Código Civil (Lei 10.406/2002) |
| Parte | Parte Geral |
| Livro | — |
| Título | Do Adimplemento e Extinção das Obrigações |
| Capítulo | Do Pagamento |
| Seção | Daqueles a Quem se Deve Pagar |
| Grupo temático | Obrigações — Modalidades e Transmissão |
| Natureza | regra |

> [!tip] Para o Estudo
> Identifique o tipo de obrigação: dar, fazer, não fazer; divisível × indivisível; solidária × conjunta. A solidariedade não se presume (art. 265).

> [!warning] Atenção — Pegadinha
> Mora do credor (mora accipiendi) x mora do devedor (mora debendi): efeitos distintos.

## Links Relacionados

← [[Art. 309]] | [[INDEX — Código Civil]] | [[Art. 311]] →
