...
# -*- coding: utf-8 -*-
"""
Enriquecedor de Tags CC: Adiciona tags específicas para parágrafos, incisos e alíneas
...
class EnriquecedorTagsCC:
    ...
    def extrair_paragrafos(self, redacao: str) -> List[str]:
        """Extrai parágrafos numerados (§1, §2) e parágrafo único"""
        tags = [f"paragrafo-{m}" for m in re.findall(r"§\s*(\d+)", redacao)]
        if re.search(r"[Pp]ar[áa]grafo\s+[úu]nico", redacao):
    ...
    def processar_todos(self, limit=None):
        """Processa todos os arquivos CC"""
        print("\n" + "="*60)
        print("🔧  ENRIQUECEDOR DE TAGS CC")
        print("="*60 + "\n")
        ...
        print(f"\n{'='*60}")
        print(f"✅ ENRIQUECIMENTO CC CONCLUÍDO!")
        print(f"{'='*60}")
        print(f"\n📊 ESTATÍSTICAS:")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Enriquecedor de Tags CC")
    ...
    base = Path(r"C:\Users\oicon\OneDrive\Allah - Islamismo\Stand Alone Legaltech\Fenice brain\FENICE bRain\02_DIREITO_PRIVADO\DIREITO_CIVIL\Artigos")

    enriquecedor = EnriquecedorTagsCC(base)
