...
class FeniceRAG:
    ...
    async def buscar_hibrido(
        ...
        return await self._buscar_async(query, limite, modo)

    async def _buscar_async(self, query: str, limite: int, modo: str) -> List[Dict]:
        async with httpx.AsyncClient(timeout=12) as client:
            if modo in ("fts", "hibrido"):
                return await self._buscar_fts(client, query, limite)
            elif modo == "keyword":
                return await self._buscar_keyword(client, query, limite)
            else:
                raise ValueError(f"Modo desconhecido: '{modo}'.")

    async def _buscar_fts(self, client: httpx.AsyncClient, query: str, limite: int) -> List[Dict]:
    ...
    async def _buscar_keyword(self, client: httpx.AsyncClient, query: str, limite: int) -> List[Dict]:
    ...
    async def responder_com_contexto(
        self,
        pergunta: str,
        prompt_template: str,
        limite: int = 5,
        ...
        resultados = await self.buscar_hibrido(pergunta, limite=limite, modo=modo)
...
async def _main_cli():
    ...
    with FeniceRAG() as rag:
        ...
        resultados = await rag.buscar_hibrido(termos, limite=3)
