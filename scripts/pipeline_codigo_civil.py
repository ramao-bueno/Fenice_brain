#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Pipeline completo Código Civil: extrai HTML → gera markdown → salva no vault."""
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent))

from config_codigo_civil import LEIS_NACIONAIS, OUTPUT_BASE
from extrator_html_generico import extrair_artigos_html
from markdown_generator_cc import MarkdownGeneratorCC
...
def processar_lei(sigla: str, cfg: dict) -> int:
    ...
    # Gera notas markdown
    gen = MarkdownGeneratorCC(OUTPUT_BASE)
    salves = 0

    for artigo in artigos:
        artigo["categoria"] = cfg.get("categoria", "DIREITO_CIVIL")
