#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Pipeline completo CPP: extrai HTML → gera markdown → salva no vault."""
import sys
from pathlib import Path
sys.path.insert(0, str(Path(__file__).parent))

from config_codigo_processo_penal import LEIS_NACIONAIS, OUTPUT_BASE
from extrator_html_generico import extrair_artigos_html
from markdown_generator_cpp import MarkdownGeneratorCPP
...
def processar_lei(sigla: str, cfg: dict) -> int:
    ...
    # Gera notas markdown
    gen = MarkdownGeneratorCPP(OUTPUT_BASE)
    salves = 0

    for artigo in artigos:
        artigo["categoria"] = cfg.get("categoria", "PROCESSO_PENAL")
...
def main():
    print("=" * 70)
    print("PIPELINE CÓDIGO DE PROCESSO PENAL")
    print("=" * 70)
    print()
